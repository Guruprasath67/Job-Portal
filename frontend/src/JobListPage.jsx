  ...
  <header className="border-b bg-white">
    ...
    <main className="mx-auto w-full max-w-7xl px-6 py-8 flex-1">
      ...
      <div className="space-y-4">
        {/* <!-- Job Card --> */}
        {jobs.length == 0 ? (
          <p className='text-grey-500'>No Jobs Available</p> )
          : jobs.map((job,index) => (

        <div key={index} className="rounded-lg border bg-white p-5 transition hover:shadow-md">
          <h3 className="text-lg font-semibold text-blue-700">{job.title}</h3>
          <p className="mt-1 text-sm text-gray-700">{job.company}</p>
          <p className='text-grey-700 text-xs'>{new Date (job.posted_on).toLocaleDateString("en-US", {
             month: "short",
              day: "numeric",
               year: "numeric"
                })}</p>
          <div className="mt-3 flex flex-wrap gap-3 text-sm text-gray-600">
            ...
            <span className="rounded bg-gray-100 px-2 py-1">🕒Full Time{job.type}</span>
          </div>
          <div className="mt-4 flex justify-end">
            <button className="text-sm font-medium text-blue-700 hover:underline" onClick={()=>handleView(job.id)}>View Details →</button>
          </div>
        </div>
            ))}
